...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    p.add_argument("--state", type=str, default="down",
                   help="initial coherent state: up (=|S,+S>, all excited) or down "
                        "(=|S,-S>, all ground); default down (legacy campaigns)")
    p.add_argument("--ckpt-dir", type=str, default=None,
                   help="directory for resume state; enables mid-trajectory "
                        "checkpointing so a job killed at its time limit can be "
                        "resumed by resubmitting (default: off)")
    p.add_argument("--ckpt-every", type=int, default=1_000_000,
                   help="integration steps between checkpoint writes "
                        "(only used when --ckpt-dir is set)")
...
def main(argv=None) -> None:
    ...
    ps = btc.collective_qfi_powersums(
        ...
        scheme="diffusive", sampling="wigner_cone", state=args.state,
    ...
    np.savez(
        ...
        scheme="diffusive", sampling="wigner_cone", state=args.state, method="semiclassical_diffusive",
    )
